...
const actions = {
    async SUBMIT_TRANSACTION_HISTORY_BY_PORTFOLIO({commit},value){
        ...
        try {
            ...
            .get(
            "/api/v1/transaction_history_portfolio_add/"+token+'/'+user_id+'/'+user_portfolio_id+'/'+amount+'/'+payment_medium+'/'+payment_date+'/'+other_information
            );
    ...
    async GET_PAYMENT_HISTORY_FROM_PORTFOLIO_ID({commit},value){
        ...
        try {
            response = await Vue.http
            .get(
            "/api/v1/transaction_history/portfolio/"+user_portfolio_id+'/'+token
            );
