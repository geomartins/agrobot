...
export default {
  ...
  methods: {
    ...
    api_params2: function(){
      return { token: this.$store.getters.getToken, id: this.$route.params.id, params: this.formData };
    },
    async api_calls(){
      ...
      response = await this.$store.dispatch('users_list_edit', this.api_params());

     if(response.success){
        this.formData = response.success;
      }
    ...
    async api_calls2(){
      ...
      // Start loader gif
      this.toggle_loading(true);
      response = await this.$store.dispatch('users_list_edit_confirm', this.api_params2());
